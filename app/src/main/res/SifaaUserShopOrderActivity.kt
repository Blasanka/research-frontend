...
import SifaaAdapters.SifaaOrderItemAdapter
...
class SifaaUserShopOrderActivity : AppCompatActivity(), SifaaOrderItemAdapter.OnItemClickListener, TimePickerDialog.OnTimeSetListener {

    private lateinit var itemRecyclerView: RecyclerView
    private lateinit var sifaaAdapter: SifaaOrderItemAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ImageView>(R.id.user_menu_order_help_iv).setOnClickListener {
            startActivity(Intent(this, SifaaContactActivity::class.java))
        }
    }
    ...
    private fun loadRecyclerAdapter() {
        ...
        val orderedItemSifaas: ArrayList<SifaaCartItem> = ArrayList()

        for (item in DatabaseHandler(this).readCartData()) {
            orderedItemSifaas.add(item)
        }

        itemRecyclerView = findViewById(R.id.order_recycler_view)
        sifaaAdapter = SifaaOrderItemAdapter(
            applicationContext,
            orderedItemSifaas,
            totalItemsTV,
            totalItems,
        ...
        itemRecyclerView.adapter = sifaaAdapter
        itemRecyclerView.layoutManager = LinearLayoutManager(this)

        sifaaAdapter.notifyItemRangeInserted(0, orderedItemSifaas.size)
    }
    ...
    override fun emptyOrder() {
        AlertDialog.Builder(this)
                .setMessage("Your order is now empty. Add some items from the flower shop and place the order.")
