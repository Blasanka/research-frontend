...
class SIfaaUserLoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.sifaa_activity_user_login)
        ...
        databaseRef.child("employees")
            .child(user.uid).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if(empGender == "none") {
                        val intent = Intent(this@SIfaaUserLoginActivity, SifaaGenderSelectActivity::class.java)
                        intent.putExtra("name", empName)
                        intent.putExtra("uid", user.uid)
                        startActivity(intent)
                        finish()
                    } else {
                        startActivity(Intent(this@SIfaaUserLoginActivity, MainActivity::class.java))
                        finish()
                        Toast.makeText(this@SIfaaUserLoginActivity, "Welcome to SIFAA", Toast.LENGTH_SHORT).show()
                    }
                }
    ...
    fun openRegisterActivity(view: View) {
        startActivity(Intent(this, SifaaRegisterUserActivity::class.java))
        finish()
    }

    private fun userForgotPassword() {
        val forgotDialog = SifaaForgotPassword()
