...
class SifaaRegisterUserActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.sifaa_activity_register)
        ...
        fullNameTextInputLayout = findViewById(R.id.register_full_name_til)
    ...
    private fun validateName(): Boolean {
        val fullName = fullNameTextInputLayout.editText!!.text.toString().trim()
        if(fullName.isEmpty()) {
            fullNameTextInputLayout.error = getString(R.string.field_empty)
            return false
        }
    ...
    private fun registerEmployee() {
        ...
        val name = fullNameTextInputLayout.editText!!.text.toString()

        registerProgressDialog.setTitle("Registering...")
        registerProgressDialog.setMessage("Creating your SIFAA account")
        ...
        reference.putFile(employeeIDCardUri)
            .addOnSuccessListener { _ ->
                ...
                reference.downloadUrl.addOnSuccessListener { uri ->
                    val model = SifaaIdCardModel(uri.toString())
    ...
    private fun addEmployeeDetailsToDatabase(user: FirebaseUser, sifaaIdCardDownloadUri: SifaaIdCardModel) {
        ...
        val consumer = databaseRef.child(user.uid)
        consumer.child("organization").setValue(orgName)
        consumer.child("emp_id").setValue(empIDNo)
        consumer.child("mobile_no").setValue(mobileNo)
        consumer.child("gender").setValue("none")
        consumer.child("reg_id").setValue(getRegID())
        consumer.child("reg_date").setValue(getRegDate())
        consumer.child("emp_id_card_uri").setValue(sifaaIdCardDownloadUri)
    ...
    private fun getRegID(): String {
        val empName = fullNameTextInputLayout.editText!!.text.toString().split(" ")
